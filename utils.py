    ...
    def after_train_epoch(self, runner):
        ...
        if runner.log_buffer.ready:
            ...
            try:
                ...
                runner.logger.info('RayIoU@1: %.4f' % metrics['val/RayIoU@1'])
                runner.logger.info('RayIoU@2: %.4f' % metrics['val/RayIoU@2'])
                runner.logger.info('RayIoU@4: %.4f' % metrics['val/RayIoU@4'])
            except:
                runner.logger.info('Not evaluate RayIoU')
            try:
                runner.logger.info('mIoU: %.4f' % metrics['val/mIoU'])
                runner.logger.info('IoU: %.4f' % metrics['val/IoU'])
            except:
                runner.logger.info('Not evaluate mIoU')
            try:
                runner.logger.info('Depth evaluation metrics:')
                runner.logger.info('CAM_FRONT: %s' % np.array2string(np.array(metrics['val/CAM_FRONT']), precision=4, separator=', '))
                runner.logger.info('CAM_FRONT_LEFT: %s' % np.array2string(np.array(metrics['val/CAM_FRONT_LEFT']), precision=4, separator=', '))
                runner.logger.info('CAM_FRONT_RIGHT: %s' % np.array2string(np.array(metrics['val/CAM_FRONT_RIGHT']), precision=4, separator=', '))
                runner.logger.info('CAM_BACK: %s' % np.array2string(np.array(metrics['val/CAM_BACK']), precision=4, separator=', '))
                runner.logger.info('CAM_BACK_LEFT: %s' % np.array2string(np.array(metrics['val/CAM_BACK_LEFT']), precision=4, separator=', '))
                runner.logger.info('CAM_BACK_RIGHT: %s' % np.array2string(np.array(metrics['val/CAM_BACK_RIGHT']), precision=4, separator=', '))
                runner.logger.info('Average: %s' % np.array2string(np.array(metrics['val/average']), precision=4, separator=', '))
            except:
                runner.logger.info('Not evaluate depth')
...
class MyTensorboardLoggerHook(LoggerHook):
    def __init__(self, log_dir=None, interval=10, ignore_last=True, reset_flag=False, by_epoch=True):
        ...
        if 'ZEEKR_TB_DIR' not in os.environ:
            self.log_dir = log_dir
        else:
            self.log_dir = os.environ['ZEEKR_TB_DIR']
            print("change to ZEEKR_TB_DIR:", self.log_dir)        
    ...
    def log(self, runner):
        ...
        for key, value in tags.items():
            ...
            if 'CAM' in key or 'average' in key:
                ignore = True
