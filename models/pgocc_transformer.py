...
class PGOccTransformer(BaseModule):
    def __init__(self,
                 embed_dims=None,
                 layers_scale=None,
                 num_queries=None,
        ...
        self.gaussians_decoder = SparseGaussiansDecoder(
            embed_dims=embed_dims,
            layers_scale=layers_scale,
    ...
    def init_weights(self):
        self.gaussians_decoder.init_weights()
    ...
    def forward(self, mlvl_feats, img_metas, depth):
        ...
        lidar2img = torch.from_numpy(lidar2img).to(feat.device)
        ego2lidar = np.asarray([m['ego2lidar'] for m in img_metas]).astype(np.float32)
        ego2lidar = torch.from_numpy(ego2lidar).to(feat.device)
        img_metas[0]['ego2img'] = torch.matmul(lidar2img, ego2lidar)

        gau_preds = self.gaussians_decoder(mlvl_feats, img_metas=img_metas, depth=depth)
...
class MaskFormerSampling(BaseModule):
    ...
    def inner_forward(self, query_feat, valid_map, occ_loc, mlvl_feats, img_metas):
        ...
        offset = self.offset(query_feat).view(B, Q, self.num_groups * self.num_points, 3)
