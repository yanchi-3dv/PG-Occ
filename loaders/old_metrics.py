...
class Metric_mIoU():
    ...
    def count_miou(self):
        ...
        if self.num_classes == 18:
            mIoU_without_others = np.delete(mIoU.copy(), [0, 12])
            print(f'===> mIoU of {self.cnt} without others samples: ' + str(round(np.nanmean(mIoU_without_others[:len(mIoU_without_others)-1]) * 100, 2)))
            return round(np.nanmean(mIoU_without_others[:len(mIoU_without_others)-1]) * 100, 2)
        else:
            return round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)

def main_miou(sem_pred_list, sem_gt_list, gt_mask_camera_list, occ_class_names):
    ...
    for sem_pred, sem_gt, mask_camera in tqdm(zip(sem_pred_list, sem_gt_list, gt_mask_camera_list), ncols=50):
        eval_metrics_miou.add_batch(sem_pred, sem_gt, None, mask_camera)
        eval_metrics_iou.add_batch(sem_pred, sem_gt, None, mask_camera)

    miou = eval_metrics_miou.count_miou()
    iou = eval_metrics_iou.count_miou()
