...
from configs.pgocc import occ_class_names as gts_class_names
...
class NuSceneOcc(NuScenesDataset):    
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        ...
        if len(inst_preds) > 0:
            ...
            from lib.eval.old_metrics import main_miou
            results = {}
            if 'rayiou' in self.metric:
                results.update(main_rayiou(occ_preds, occ_gts, lidar_origins, occ_class_names=occ_class_names))
            if 'miou' in self.metric:
                results.update(main_miou(occ_preds, occ_gts, cam_masks, occ_class_names=occ_class_names))
            return results
