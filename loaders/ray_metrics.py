...
def process_one_sample(sem_pred, lidar_rays, output_origin, instance_pred=None, occ_class_names=None):
    ...
    for t in range(T): 
        ...
        pred_label = sem_pred[coord_index[:, 0].long(), coord_index[:, 1].long(), coord_index[:, 2].long()][:, None]  # [N, 1]        
        pred_dist = pred_dist[0, :, None].cpu()

        if instance_pred is not None:
            pred_instance = instance_pred[coord_index[:, 0].long(), coord_index[:, 1].long(), coord_index[:, 2].long()][:, None]  # [N, 1]
